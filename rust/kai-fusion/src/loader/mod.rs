...
use crate::config::{AttnKind, AttnPolicy, MlpKind, MoEConfig, MLAConfig, VisionConfig};
...
/// Parse tensor info from an already-loaded byte buffer.
pub fn parse_from_bytes(data: &[u8], _path: &str) -> Result<(u32, Vec<TensorInfo>, u64), String> {
    if data.len() < 16 {
...
/// Parse metadata KV pairs from already-loaded GGUF bytes.
/// This is like read_kv() but operates on a byte slice instead of re-reading the file.
pub fn read_kv_from_bytes(data: &[u8]) -> Result<HashMap<String, GgufMeta>, String> {
    if data.len() < 16 {
        return Err("file too small".into());
    }
    let mut p = 0usize;
...
pub struct GgufBuffer {
    ...
    pub data_start: u64,
    pub meta: HashMap<String, GgufMeta>,
}

impl GgufBuffer {
    /// Memory-map a GGUF file, parse tensor index and metadata (lazy disk access).
    pub fn open(path: &str) -> Result<Self, String> {
        let file = std::fs::File::open(path).map_err(|e| format!("open {path}: {e}"))?;
        let bytes = unsafe { Mmap::map(&file).map_err(|e| format!("mmap {path}: {e}"))? };
        let (version, tensors, data_start) = parse_from_bytes(&bytes, path)?;
        let meta = read_kv_from_bytes(&bytes)?;
        Ok(GgufBuffer { bytes, version, tensors, data_start, meta })
    }
    ...
    /// NOTE: read-only mmap — overwrites are no-ops (returns Ok for inference).
    pub fn overwrite(&mut self, name: &str, data: &[f32]) -> Result<(), String> {
        ...
        // read-only mmap: skip actual write (model files are immutable)
        Ok(())
    }
...
pub fn build_config(meta: &HashMap<String, GgufMeta>) -> Option<Config> {
    ...
    let vocab_size = get("vocab_size")
        .or_else(|| get("n_vocab"))
        .or_else(|| {
            // Fallback: length of tokenizer.ggml.tokens string array
    ...
    Some(Config {
        ...
        base_ms: 1000.0,
        phi: 0.0,
        // Architecture policy (Slice 1 boundary)
